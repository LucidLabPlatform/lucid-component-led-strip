"""
Rainbow cycle effect — distributes the full rainbow evenly across all pixels.

Unlike the basic rainbow effect, every hue is always visible at once.
    ...
    while not cancel_event.is_set():
        for i in range(hardware.LED_COUNT):
            hardware.set_pixel_color(
                i,
                hardware.wheel((int(i * 256 / hardware.LED_COUNT) + j) & 255),
            )
        hardware.show()
        j = (j + 1) % 256
